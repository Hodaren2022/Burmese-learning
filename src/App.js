...
function App() {
  ...
  function speak(textToSpeak) {
    ...
    const url = isLocal
      ? `http://localhost:4001/tts?q=${encodeURIComponent(textToSpeak)}`
    ...
    try {
      ...
      a.play().then(() => {
        ...
        console.warn('Audio fallback play() rejected, using audioRef fallback', err);
        if (audioRef.current) {
      ...
      console.warn('Audio fallback creation failed, trying audioRef', e);
      if (audioRef.current) {
        audioRef.current.src = url;
        audioRef.current.play().catch((err) => console.warn('audioRef play failed', err));
      }
    }
  }
  ...
  function renderSentenceCards() {
    return Object.keys(categorizedSentences).map((cat) => (
      <div key={cat} className="">
        <h3 style={{ margin: '10px 0 6px 0' }}>{cat}</h3>
        <div className="sentence-list">
          {categorizedSentences[cat].map((s, i) => (
    ...
    <div className="app-root">
      ...
      <div className="app-container">
        ...
        <main className="app-main">
          ...
          {page === 'sentences' && (
            <div style={{ marginTop: 18 }}>{renderSentenceCards()}</div>
          )}
        </main>
      </div>
    </div>
